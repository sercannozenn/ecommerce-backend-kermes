...
class ProductDiscountUpdateRequest extends FormRequest
{
    /**
        ...
        return [
            ...
            'discount_start'  => ['required', 'date',
                function ($attribute, $value, $fail) use ($today) {
                    $existing = $this->route('product_discount'); // route model binding ile gelir
                    $existingStart = $existing->discount_start->toDateString(); // Yani "2025-04-23"
                    $submittedStart = Carbon::parse($value)->toDateString();    // Yani "2025-04-25"

                    if ($existing->discount_start->lt(Carbon::today()) && $submittedStart !== $existingStart) {
                        $fail('Başlangıç tarihi geçmişte olduğu için artık değiştirilemez.');
                    }

                    if ($existing->discount_start >= $today && $value < $today) {
                        $fail('Başlangıç tarihi en az bugünün tarihi olmalı.');
                    }
                }
